            ...
            match base_tag {
                ...
                "a" => {
                    ...
                    if let Some(start) = current_tag.find("href=\"") {
                        ...
                        if let Some(end) = rest.find('"') { href = &rest[..end]; }
...
pub fn load_chapter(archive: &mut ZipArchive<File>, path: &str, wrap_width: usize, margin_left: usize) -> Vec<String> {
    ...
    for line in clean.lines() {
        ...
        if trimmed.contains('\x1e') {
            ...
            let pad = if wrap_width > visible_len { (wrap_width - visible_len) / 2 } else { 0 };
